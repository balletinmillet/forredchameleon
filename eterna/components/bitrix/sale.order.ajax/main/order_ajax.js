...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		initializePrimaryFields: function()
		{
			...
			this.userAuthStatusNodeName = 'ORDER_PROP_15';
			this.phoneMask = "+7(999)999-99-99";
			this.courierDeliveryId = '5';
			this.pickupDeliveryId = '6';
			this.locationPropertyIds = ['7','8','9','10'];
			this.freeDeliverydDefaultValue = "10000";
			this.kladrCodes = [];
			this.lamodaDeliveries = [];
			this.lamodaDeliveryBlockInit = false;
			this.lamodaDeliveryConfirm = false;
			this.mindboxEmailError = false;
			this.mindboxPhoneError = false;
			this.mindboxUndefinedError = false;
			this.totalOrderPrice = false;
		...
		init: function(parameters)
		{
			...
			this.customResult = parameters.customResult || {};
		...
		customInitBefore: function() {},
		customInitAfter: function()
		{
			this.kladrCodes["CITY"] = "";
			this.kladrCodes["STREET"] = "";
			this.kladrCodes["BUILDING"] = "";

			var userAuthStatusNode = this.propsBlockNode.querySelector('[name="' + this.userAuthStatusNodeName +'"]');
			if(userAuthStatusNode)
				userAuthStatusNode.value = this.result.IS_AUTHORIZED ? "Y" : "N";

			this.pickupPointButtonBindEvent();

			for (let i = 0; i < this.result.DELIVERY.length; i++)
			{
				if(this.result.DELIVERY[i].CHECKED == "Y")
				{
					this.setDeliverySettings(this.result.DELIVERY[i].ID);
					break;
				}
			}
		...
		showPickupPoints: function(event)
		{
			if(BX.PickupMap.availableForm)
			{
				this.setPickupStatus("hide");
				this.showPickupMap();
			}
			else
			{
		...
		getPickupErrorMessageBlock: function()
		{
			return BX.create('DIV', {
				props: {className: 'error-message-container pickup-error-container'},
				children: [
					BX.create('span', {
						props: {className: 'error-message'},
						text: 'К сожалению, ПВЗ в вашем регионе не найдены'
					})
				]
			});
		},

		setPickupStatus: function(state = "show")
		{
			var pickupButton, companyContainer, pickupErrorContainer;

			if(state == "show")
			{
				pickupErrorContainer = this.deliveryBlockNode.querySelector('.pickup-error-container');
				if(pickupErrorContainer)
					return;

				pickupButton = this.deliveryBlockNode.querySelector('.pickup-button');
				if(pickupButton)
				{
					companyContainer = BX.findParent(pickupButton, {className: "bx-soa-pp-company"});
					if(companyContainer)
						companyContainer.appendChild(this.getPickupErrorMessageBlock());
				}
			}
			else
			{
				pickupErrorContainer = this.deliveryBlockNode.querySelector('.pickup-error-container');
				if(pickupErrorContainer)
					BX.remove(pickupErrorContainer);
			}
		},
		...
		refreshOrder: function(result)
		{
			...
			this.customRefreshOrder();

			return true;
		},

		customRefreshOrder: function()
		{
			var property, locationInput, kladrCode, deliveryId,
				propertyKeys = this.customResult.LOCATION.INPUTS.LIST_ID;

			for (let i = 0; i < this.result.DELIVERY.length; i++)
			{
				if(this.result.DELIVERY[i].CHECKED == "Y")
				{
					deliveryId = this.result.DELIVERY[i].ID;
					this.setDeliverySettings(this.result.DELIVERY[i].ID);
					break;
				}
			}
			...
			if(deliveryId == '5')
			{
				...
				{
					locationInput = BX(propertyKeys[i]);
					if(locationInput)
					{
						property = this.customResult.LOCATION.INPUTS[propertyKeys[i]];
						kladrCode = this.kladrCodes[property['CODE']];
						if(kladrCode)
						{
							locationInput.setAttribute('data-location-id', kladrCode);
							locationInput.setAttribute('data-location-type', property['CODE']);
						}
					}
				}

				if(this.lamodaDeliveries.length > 0 && this.customResult.selectedDeliveryId != null)
					this.createLamodaDeliveryBlock(this.lamodaDeliveries, this.customResult.selectedDeliveryId);
			}


			if(this.totalOrderPrice != this.result.TOTAL.ORDER_TOTAL_PRICE)
			{
				if(this.kladrCodes["CITY"] != "")
					BX.PickupMap.initPickupPoints(this.kladrCodes["CITY"], Object.values(this.result.GRID.ROWS).length, this.result.TOTAL.ORDER_PRICE);

				this.totalOrderPrice = this.result.TOTAL.ORDER_TOTAL_PRICE;
			}

			try {$(".field.phone input").mask(this.phoneMask);} catch (e) {}
		},

		setDeliverySettings: function(deliveryId)
		{
			switch (deliveryId)
			{
				case this.courierDeliveryId:
					this.setCourierDeliverySettings();
					break;
				case this.pickupDeliveryId:
					this.setPickupDeliverySettings();
					break;
			}
		},

		setCourierDeliverySettings: function()
		{
			for (let i = 0; i < this.locationPropertyIds.length; i++)
			{
				let prop = this.propsBlockNode.querySelector('[name="ORDER_PROP_' + this.locationPropertyIds[i] + '"]');
				if(prop)
					BX.removeClass(prop,'disabled-input')
			}
		},

		setPickupDeliverySettings: function()
		{
			for (let i = 0; i < this.locationPropertyIds.length; i++)
			{
				let prop = this.propsBlockNode.querySelector('[name="ORDER_PROP_' + this.locationPropertyIds[i] + '"]');
				if(prop)
					BX.addClass(prop,'disabled-input')
			}
		},

		setInputValue: function(inputSelector, inputValue)
		{
			let input = this.propsBlockNode.querySelector(inputSelector);
			if(input)
				input.value = inputValue;
		},

		selectPickupDelivery: function(deliveryInfo)
		{
			this.setInputValue('[name="ORDER_PROP_7"]', deliveryInfo.street);
			this.setInputValue('[name="ORDER_PROP_8"]', deliveryInfo.house);
			this.result.pickupPointMapPopup.close();

			let checkoutMethodDeliveryPrice = deliveryInfo.serviceLevelTypes[0].deliveryPrice;
			let checkoutMethodFreeDeliveryNetThreshold = deliveryInfo.serviceLevelTypes[0].freeDeliveryNetThreshold;
			let intervalId = deliveryInfo.serviceLevelTypes[0].availableDays[0].availableIntervals[0].id;

			let selectedDelivery = {
				"checkoutMethodName": deliveryInfo.name,
				"checkoutMethodDeliveryPrice": checkoutMethodDeliveryPrice,
				"checkoutMethodFreeDeliveryNetThreshold": checkoutMethodFreeDeliveryNetThreshold ? checkoutMethodFreeDeliveryNetThreshold
					: this.freeDeliverydDefaultValue,
		...
		showStockError: function(unavailableBasketItems)
		{
			var errorContainer = this.totalBlockNode.querySelector('.alert.alert-danger');
			if (errorContainer)
				BX.remove(errorContainer);

			var totalCart  = this.totalBlockNode.querySelector('.bx-soa-cart-total');
			if(totalCart)
				totalCart.appendChild(BX.create('DIV', {
					props: {className: 'alert alert-danger'},
					html: "В корзине находятся товары которых нет на складе"
				}));
		...
		showError: function(node, msg, border)
		{
			if(msg[0] !== 'undefined')
			{
				if(msg[0]["ERROR_CODE"] !== 'undefined')
				{
					if(msg[0]["ERROR_CODE"] == "UNAVAILABLE_IN_STOCK")
						return this.showStockError(msg[0]["UNAVAILABLE_GOODS"]);
				}
			}
...
/*				case this.basketBlockNode.id:
		...
		showMindboxUndefinedError: function()
		{
			this.mindboxUndefinedError = true;
			this.isValidForm();
		},
		...
		doSaveAction: function()
		{
			...
			{
				this.customDoSaveAction();
			}
		},

		customDoSaveAction: function()
		{
			if (this.isOrderSaveAllowed())
			{
				this.reachGoal('order');
...
/*			var node = !!activeNodeMode ? this.basketBlockNode : this.basketHiddenBlockNode,
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-item-container styled-chooser s1 '}}),
				paySystemItemNode, i;
		...
		pickupPointButtonBindEvent: function()
		{
			var pickupPointButton = this.deliveryBlockNode.querySelector('[name="PICKUP_POINT_BUTTON"]');
			if(pickupPointButton)
				BX.bind(pickupPointButton, 'click', BX.proxy(this.showPickupPoints, this));
		},
		...
		createPaySystemItem: function(item)
		{
			...
			logoNode = BX.create('DIV', {props: {className: ''}});
			...
			{
				title = BX.create('DIV', {props: {className: 'item-title'}, text: item.NAME});
			}

			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-12 col-sm-12 col-xs-12 item'},
...
/*			paySystemInfoContainer.appendChild(
		...
		clearLocationFields: function(selectedDelivetyId)
		{
			if(selectedDelivetyId == '6')
			{
				this.removeDeliveryBlock();
				this.clearDelivery();
				//this.customResult.selectedDeliveryId = null;
				this.lamodaDeliveries = [];
				this.lamodaDeliveryConfirm = false;
				this.kladrCodes["STREET"] = "";
				this.kladrCodes["BUILDING"] = "";
			...
			var locationPropertyIds = ['7','8','9','10'];
			for (let i = 0; i < locationPropertyIds.length; i++)
			{
				let input = this.propsBlockNode.querySelector('[name="ORDER_PROP_' + locationPropertyIds[i] + '"]');
				if(input)
					input.value = "";
			}

			console.log(selectedDelivetyId);
			console.log(this.kladrCodes);
		},

		clearDelivery: function()
		{
			this.sendCustomRequest('clearDelivery', {arFields: {'clearDelivery': 'true'}}, {"app": this});
		},
...
/*			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-item-container styled-chooser s1 hidden-block'}}),
				deliveryItemNode, k;*/

			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-item-container styled-chooser s1'}}),
				deliveryItemNode, k;
			...
			var lamodaDeliveryItemsContainer = BX.create('DIV', {props: {id: 'soa-lamoda-deliveries', className: 'col-sm-12 bx-soa-pp-item-container styled-chooser s1'}});

			deliveryNode.appendChild(deliveryItemsContainer);
			deliveryNode.appendChild(lamodaDeliveryItemsContainer);
		},
...
/*				labelNodes.push(
			...
			if(deliveryId == 6)
			{
				buttons = BX.create('DIV', {
					props: {className: 'pp-button-block pickup-button-container'},
					children: [
						BX.create('A', {
							props: {className: 'btn styled-button pickup-button', name: 'PICKUP_POINT_BUTTON',},
							text: 'Выбрать ПВЗ',
						}),
						BX.create('DIV', {
							props: {className: 'button-await'},
							children: [
/*								BX.create('DIV', {
									props: {className: 'button-loader'},
									children: [
		...
		setPickupButtonDisplay: function(state = "default") {

			var pickupButtonContainer = this.deliveryBlockNode.querySelector('.pickup-button-container');
			if(pickupButtonContainer)
			{
				if(state == "default")
					BX.removeClass(pickupButtonContainer, 'pickup-awaiter');
				else if(state == "await")
					BX.addClass(pickupButtonContainer, 'pickup-awaiter');
			}
		},

		setMapScreenType: function(screenType = "full") {

			var mapContainer, fullscreenControl;
			mapContainer = BX('pickup-form');
			if(mapContainer)
			{
				fullscreenControl = mapContainer.querySelector('.gm-fullscreen-control');
				if(fullscreenControl && screenType == "full")
				{
					if(!BX.hasClass(fullscreenControl, 'fullscreen-active'))
					{
						BX.addClass(fullscreenControl,"fullscreen-active");
						fullscreenControl.click();
					}
				}
				if(fullscreenControl && screenType == "min")
				{
					if(BX.hasClass(fullscreenControl, 'fullscreen-active'))
					{
						BX.removeClass(fullscreenControl,"fullscreen-active");
						fullscreenControl.click();
					}
				}
			}
		...
		showPickupMap: function() {
			...
			var app = this,
				popupId = this.isMobile ? 'soa-google-map-popup-mobile' : 'soa-google-map-popup';

			this.result.pickupPointMapPopup = BX.PopupWindowManager.create(popupId, null, {
				autoHide: true,
				offsetLeft: 0,
				offsetTop: 0,
				overlay : true,
				...
				content: BX("bx-soa-delivery-pickup"),
				events: {
			...
			this.result.pickupPointMapPopup.show();
		},
...
/*
		...
		selectDelivery: function(event)
		{
			...
			this.clearLocationFields(selectedInput.value);
			this.sendRequest();
		},
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
				...
				label = BX.create('DIV', {
					...
					props: {className: 'bx-soa-custom-label label col-12 col-sm-3 col-lg-4 my-auto ' + requiredClass },
					html: textHtml
				});
		...
		{
			var prop, inputs, values, i, propContainer, propertyAdditionalClasses;
			...
			{
				propertyAdditionalClasses = property.getId() == 3 ? ' field phone' : ''
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container input-wrapper col-12 col-sm-9 col-lg-8' + propertyAdditionalClasses}});
				property.appendTo(propContainer);
				propsItemNode.appendChild(propContainer);
				this.alterProperty(property.getSettings(), propContainer);
				this.bindValidation(property.getId(), propContainer);
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if(this.customResult.LOCATION.INPUTS.LIST_ID.includes(textNode.id))
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				if(this.customResult.LOCATION.INPUTS.LIST_ID.includes(inputs[i].id))
					BX.bind(inputs[i], 'keyup', BX.proxy(this.getLocation, this));
		...
		isValidForm: function()
		{
			...
			if(!this.lamodaDeliveryConfirm)
			{
				if(propsErrors.length == 0)
					propsErrors.push("К сожалению, в данный момент мы не осуществляем доставку в ваш регион.");
			}

			if(propsErrors.length == 0)
			{
				if(this.mindboxEmailError == true)
					propsErrors.push("Пользователь с указанным емейлом уже существует. Попробуйте оформить заказ после авторизации.");

				if(this.mindboxPhoneError == true)
					propsErrors.push("Пользователь с указанным телефоном уже существует. Попробуйте оформить заказ после авторизации.");

				if(this.mindboxUndefinedError == true)
					propsErrors.push("Ошибка выполнения запроса! Сообщите администрации сайта об этой ошибке.");
			}
			...
			if (propsErrors.length && !navigated)
			{
				if (this.activeSectionId == this.propsBlockNode.id)
				{
					...
					{
						this.animateScrollTo(this.propsBlockNode, 800, 200);
					}
				}
				else
					this.animateScrollTo(this.propsBlockNode, 800, 200);
		...
		editPropsComment: function(propsNode)
		{
			...
			label = BX.create('DIV', {
				...
				props: {className: 'bx-soa-customer-label label col-12 col-sm-3 col-lg-4 my-auto'},
				html: this.params.MESS_ORDER_DESC
			});
			input = BX.create('DIV', {
				props: {className: 'soa-property-container input-wrapper col-12 col-sm-9 col-lg-8'},
				children: [
					BX.create('TEXTAREA', {
						props: {
							id: 'orderDescription',
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
									className: 'btn styled-button btn-lg btn-order-save'
								},
		...
		{
			var totalValue, className = 'bx-soa-cart-total-line calc-row';
			...
			else if (params.free)
			{
				...
				{
					value = '';
					name = 'Чтобы узнать стоимость доставки укажите, пожалуйста, адрес доставки';
					className += ' bx-soa-cart-total-not-active-line-total';
				}
			...
			return BX.create('DIV', {
				...
				children: [
					BX.create('SPAN', {props: {className: 'bx-soa-cart-t text'}, text: name}),
					BX.create('SPAN', {
						props: {
							className: 'bx-soa-cart-d value' + (!!params.total && this.options.totalPriceChanged ? ' bx-soa-changeCostSign' : '')
						},
						children: totalValue
					})
				]
		...
		getLocation: function(event)
		{
			var clickedNode = event.target || event.srcElement, property, contextParams, params,
				locationType, parentLocationId, parentInput;

			this.lamodaDeliveryBlockInit = false;
			this.selectedDeliveryId = 0;

			if(clickedNode.value.length < 1)
				return;

			if(this.customResult.LOCATION.INPUTS && clickedNode.id)
			{
				property = this.customResult.LOCATION.INPUTS[clickedNode.id];
				if(property)
					locationType = property["CODE"];
			}

			if(locationType == "STREET")
			{
				parentInput = this.propsBlockNode.querySelector('[data-location-type="CITY"]');
				if(parentInput)
					parentLocationId = parentInput.getAttribute('data-location-id');
			}

			if(locationType == "BUILDING")
			{
				parentInput = this.propsBlockNode.querySelector('[data-location-type="STREET"]');
				if(parentInput)
					parentLocationId = parentInput.getAttribute('data-location-id');
			}
...
/*		getLocationsListNode: function(list, locationType)
		...
		selectLocation: function(event)
		{
			var clickedNode = event.target || event.srcElement,
				locationId, locationName, locationType, input, parentNode, basketItems, kladrCodeIdInput;
			...
			{
				this.setPickupStatus("hide");
				BX.PickupMap.initPickupPoints(locationId, Object.values(this.result.GRID.ROWS).length, this.result.TOTAL.ORDER_PRICE);
			}
			...
			if(parentNode)
			{
				input = parentNode.querySelector('input.bx-soa-customer-input');
				if(input)
				{
					this.kladrCodes[locationType] = locationId;
					...
					kladrCodeIdInput = BX(this.customResult.PROPERTIES.KLADR_CODE)
					if(kladrCodeIdInput)
					{
						if(this.kladrCodes["BUILDING"].length > 0)
							kladrCodeIdInput.value = this.kladrCodes["BUILDING"];
						else if(this.kladrCodes["STREET"].length > 0)
							kladrCodeIdInput.value = this.kladrCodes["STREET"];
					}
				}
			}
			...
			if(locationType == "BUILDING")
			{
				basketItems = Object.values(this.result.GRID.ROWS);
				this.sendCustomRequest(
					'getDeliveries',
					{
						arFields: {
							...
							'cartAmount': this.result.TOTAL.ORDER_PRICE,
						}
					},
					{
						"app": this,
		...
		{
			var clickedNode = event.target || event.srcElement,
				selectDeliveryId, selectDelivery;
			
			selectDeliveryId = clickedNode.value;
			selectDelivery = this.lamodaDeliveries[selectDeliveryId];
			this.customResult.selectedDeliveryId = selectDeliveryId;
		...
		showLocations: function(response, contextParams)
		{
			...
			if(!response.data)
			{
				if(contextParams.locationType)
					this.kladrCodes[contextParams.locationType] = '';
			...
			listNode = this.getLocationsListNode(response.data.data, contextParams.locationType);
		...
		createLamodaDeliveryItem: function(item, itemId, checked)
		{
			...
			var deliveryId = parseInt(itemId),
				labelNodes = [
					...
					BX.create('INPUT', {
						props: {
							id: 'LAMODA_DELIVERY_ID_' + itemId,
							name: 'LAMODA_DELIVERY_ID',
							type: 'checkbox',
							className: 'bx-soa-pp-company-checkbox',
							value: itemId,
							checked: checked
				...
				logotype, label, title, itemNode, logoNode, descrNode;
			...
			if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y')
			{
				title = BX.create('DIV', {
					props: {className: 'item-title'},
					text: item.serviceLevelTypeName.length > 0 ? item.checkoutMethodName + ' ' + item.serviceLevelTypeName : item.checkoutMethodName
				});
			}

			descrNode = BX.create('DIV', {
				props: {className: 'delivery-description'},
				html: '<p><b>Доставка: </b>' + item.checkoutMethodCheckoutDescription + '</p>'
					+ '<p><b>Дата доставки: </b>' + item.dayDate + '</p>'
					+ '<p><b>Время работы: </b>' + item.intervalStart + ' - ' + item.intervalEnd + '</p>'
			});


			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-lamoda-company col-lg-12 col-sm-12 col-xs-12 item'},
				attrs: {'data-delivery-id': itemId.toString()},
				children: [label, title, descrNode],
				events: {click: BX.proxy(this.selectLamodaDelivery, this)}
			});
		...
		createLamodaDeliveryBlock: function(deliveryList, checkedItemId)
		{
			...
			if(deliveryContainer)
			{
				itemsContainer = deliveryContainer.querySelector('.soa-items-container');
				if(itemsContainer)
					BX.remove(itemsContainer);
			...
			if(!this.lamodaDeliveryBlockInit)
			{
				this.lamodaDeliveryBlockInit = true;
				this.sendCustomRequest(
					'setDelivery',
					{
						arFields: {
							'delivery': deliveryList[checkedItemId],
						}
					},
					{
						"app": this,
						"clickedNode": this.deliveryBlockNode.querySelector('div[data-delivery-id="' + checkedItemId + '"]'),
					}
				);
			}
		},

		clearLocations: function(response, contextParams)
		{
			this.lamodaDeliveryConfirm = false;
		},
		...
		showLamodaDeliveries: function(response, contextParams)
		{
			...
			this.createLamodaDeliveryBlock(response.data, 0);
			this.lamodaDeliveries = response.data;

			if(response.data.length == 0)
				this.lamodaDeliveryConfirm = false;
		},

		setSelectedDelivery: function(response, contextParams)
		{
			var deliveryIntervalIdInput;
			
			deliveryIntervalIdInput = BX(this.customResult.PROPERTIES.INTERVAL_INPUT_ID)
			if(deliveryIntervalIdInput)
				deliveryIntervalIdInput.value = response.data['INTERVAL_ID'];

			this.lamodaDeliveryConfirm = true;

			this.sendRequest();
		},

		clearDeliveryHandler: function(response, contextParams)
		{
		...
		setRequestResult: function(func, response, contextParams)
		{
			switch (func)
			{
				...
				case 'getDeliveries':
					...
					break;
				case 'setDelivery':
					this.setSelectedDelivery(response, contextParams);
					break;
				case 'clearDelivery':
					this.clearDeliveryHandler(response, contextParams);
					break;
			}
		},
		...
		{
			BX.ajax.runComponentAction('custom:eterna.order.ajax', func, {
				mode: 'class',
				data: fucnParams
